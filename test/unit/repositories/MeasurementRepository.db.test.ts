...
describe("MeasurementRepository: SQLite in-memory", () => {
    ...
    // Utility function to create dates
    ...
    describe("TS1: createMeasurement()", () => {
        ...
        it("T1.2: Error if sensor is null", async () => {
            ...
            // Note: Based on implementation, this might not throw if there's no validation
            // but the test expects it to throw
            await expect(
                repo.createMeasurement(date, value, null)
            ).rejects.toThrow();
        });
    ...
    describe("TS2: getAllMeasurement()", () => {
        ...
        it("T2.1: Should return all measurements", async () => {
            ...
            // Check if the returned measurements include all the expected data
